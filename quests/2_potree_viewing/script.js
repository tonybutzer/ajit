...
var init = (name) => {
    ...
    if (window.config.classifications) {
        Object.entries(window.config.classifications)
        .forEach(([name, { value, visible }]) => {
            console.log('Adding classification', name, value, visible)
            viewer.classifications[value] = { name, visible }
        })
    }
...
;(async () => {
    const results = await Promise.allSettled(
        resources.map((p, i) => new Promise((resolve, reject) => {
    ...
    results.forEach((r) => {
        if (r.status !== 'fulfilled') return console.log(r.reason)
        viewer.scene.addPointCloud(r.value)
    })
